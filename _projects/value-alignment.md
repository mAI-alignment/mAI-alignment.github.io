...
*Value Alignment* tackles the challenge of steering language models so their behaviour follows a **specified human value profile**, such as one defined by the Schwartz Value Theory. A core question is whether these values manifest **consistently in out-of-domain evaluations**. Our current work explores a simple approach: fine-tuning models on value survey responses to induce the desired value system, as introduced in our paper [From Value Conditioning to Behavioral Shift](https://arxiv.org/abs/2508.11414).
